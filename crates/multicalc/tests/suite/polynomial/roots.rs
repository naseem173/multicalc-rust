...
fn linear_root() {
    // 2x - 6
    let poly: Polynomial<2> = Polynomial::new([-6.0, 2.0]);
    assert_close(poly.real_roots().unwrap().as_slice(), &[3.0], 1e-12);
}
...
fn quadratic_two_roots() {
    ...
    assert_close(poly.real_roots().unwrap().as_slice(), &[1.0, 2.0], 1e-12);
}

#[test]
fn quadratic_repeated_root() {
    // (x - 3)², which reports its root twice.
    let poly: Polynomial<3> = Polynomial::new([9.0, -6.0, 1.0]);
    assert_close(poly.real_roots().unwrap().as_slice(), &[3.0, 3.0], 1e-12);
}

#[test]
fn quadratic_no_real_roots() {
    // x² + 1
    let poly: Polynomial<3> = Polynomial::new([1.0, 0.0, 1.0]);
    assert!(poly.real_roots().unwrap().is_empty());
}
...
fn quadratic_survives_a_dominant_linear_term() {
    ...
    let poly: Polynomial<3> = Polynomial::new([1.0, 1e8, 1.0]);

    let roots = poly.real_roots().unwrap();
...
fn cubic_three_real_roots() {
    // (x - 1)(x - 2)(x - 3)
    let poly: Polynomial<4> = Polynomial::new([-6.0, 11.0, -6.0, 1.0]);
    assert_close(
        poly.real_roots().unwrap().as_slice(),
        &[1.0, 2.0, 3.0],
        1e-10,
    );
}
...
fn cubic_one_real_root() {
    // x³ + x + 1
    let poly: Polynomial<4> = Polynomial::new([1.0, 1.0, 0.0, 1.0]);
    let roots = poly.real_roots().unwrap();
...
fn cubic_repeated_root() {
    ...
    let poly: Polynomial<4> = Polynomial::new(CUBIC_REPEATED);
    assert_close(
        poly.real_roots().unwrap().as_slice(),
        &[-1.0, 2.0, 2.0],
        1e-10,
...
fn quartic_four_real_roots() {
    let poly: Polynomial<5> = Polynomial::new(QUARTIC_FOUR_ROOTS);
    assert_close(
        poly.real_roots().unwrap().as_slice(),
        &[0.0, 1.0, 2.0, 4.0],
        1e-9,
...
fn quartic_two_real_roots() {
    ...
    let poly: Polynomial<5> = Polynomial::new([-2.0, 1.0, -1.0, 1.0, 1.0]);
    assert_close(poly.real_roots().unwrap().as_slice(), &[-2.0, 1.0], 1e-10);
}

#[test]
fn quartic_no_real_roots() {
    // (x² + 1)(x² + 4)
    let poly: Polynomial<5> = Polynomial::new([4.0, 0.0, 5.0, 0.0, 1.0]);
    assert!(poly.real_roots().unwrap().is_empty());
}
...
fn closed_form_roots_differentiate() {
    ...
    let poly: Polynomial<3, Dual<f64>> = Polynomial::new([
        Dual::variable(2.0),
        Dual::constant(-3.0),
        Dual::constant(1.0),
    ]);
...
fn count_real_roots_matches_the_closed_form() {
    let poly: Polynomial<5> = Polynomial::new(QUARTIC_FOUR_ROOTS);
    let bound = poly.cauchy_root_bound().unwrap();

    assert_eq!(poly.count_real_roots(-bound, bound).unwrap(), 4);
    assert_eq!(poly.real_roots().unwrap().len(), 4);

    // Roots at 0 and 1, then 2 and 4, then 4 on its own.
    assert_eq!(poly.count_real_roots(-1.0, 1.5).unwrap(), 2);
    assert_eq!(poly.count_real_roots(1.5, 5.0).unwrap(), 2);
    assert_eq!(poly.count_real_roots(2.5, 5.0).unwrap(), 1);
    assert_eq!(poly.count_real_roots(4.5, 5.0).unwrap(), 0);
}
...
fn count_real_roots_counts_distinct_roots() {
    ...
    let poly: Polynomial<4> = Polynomial::new(CUBIC_REPEATED);
    assert_eq!(poly.count_real_roots(-5.0, 5.0).unwrap(), 2);
    assert_eq!(poly.real_roots().unwrap().len(), 3);
}
...
fn real_roots_in_matches_the_closed_form_quartic() {
    ...
    let narrowed = poly.real_roots_in(-bound, bound, 1e-11, 1000).unwrap();
    assert_close(
        narrowed.as_slice(),
        poly.real_roots().unwrap().as_slice(),
        1e-9,
...
fn real_roots_in_finds_a_degree_six_polynomial() {
    ...
    let poly: Polynomial<7> = Polynomial::new([84.0, -131.0, -126.5, 104.5, 5.5, -9.5, 1.0]);

    let roots = poly.real_roots_in(-10.0, 10.0, 1e-10, 1000).unwrap();
...
fn real_roots_in_reports_running_out_of_steps() {
    let poly: Polynomial<5> = Polynomial::new(QUARTIC_FOUR_ROOTS);
    assert!(matches!(
        poly.real_roots_in(-15.0, 15.0, 1e-12, 2),
...
fn real_roots_in_rejects_bad_arguments() {
    ...
    assert_eq!(
        poly.real_roots_in(-bound, bound, f64::NAN, 1000).err(),
        Some(PolynomialError::NonFinite)
    );
    assert_eq!(
        poly.real_roots_in(f64::INFINITY, bound, 1e-11, 1000).err(),
        Some(PolynomialError::NonFinite)
    );
    ...
    assert_eq!(
        poly.real_roots_in(-bound, bound, 0.0, 1000).err(),
        Some(PolynomialError::ToleranceNotPositive)
    );
    assert_eq!(
        poly.real_roots_in(-bound, bound, -1e-11, 1000).err(),
        Some(PolynomialError::ToleranceNotPositive)
    );
    ...
    assert_eq!(
        poly.real_roots_in(-bound, bound, 1e-11, 1000)
            .unwrap()
            .len(),
        4
            );
